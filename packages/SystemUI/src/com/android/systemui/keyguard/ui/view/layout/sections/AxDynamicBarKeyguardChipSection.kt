...
constructor(
    ...
    override fun bindData(constraintLayout: ConstraintLayout) {
        ...
        val batteryInfoVal = viewModel.keyguardBatteryInfo.value
        val batteryChipModeVal = viewModel.keyguardBatteryChipMode.value
        val isDozingVal = viewModel.isDozing.value
        val isBatteryChipEnabledVal = batteryChipModeVal > 0
        val isBatteryChipVisibleVal = isBatteryChipEnabledVal && (batteryChipModeVal == 2 || (batteryChipModeVal == 1 && batteryInfoVal.isCharging))

        val stateVal = viewModel.chipState.value
        val showNormalDynamicBarVal = viewModel.isEnabled.value && viewModel.isKeyguardEnabled.value && stateVal != null
        val showLockscreenMediaVal = viewModel.isLockscreenMediaEnabled.value && stateVal != null && stateVal.event is IslandEvent.Media
        val showLockscreenLyricsVal = viewModel.isLockscreenMediaLyricsEnabled.value && stateVal != null && stateVal.event is IslandEvent.Media
        val showBatteryChipVal = stateVal == null && isBatteryChipVisibleVal && viewModel.isEnabled.value && viewModel.isKeyguardEnabled.value && !isDozingVal

        val shouldSuppress = viewModel.isOnKeyguard.value && (showNormalDynamicBarVal || showLockscreenMediaVal || showLockscreenLyricsVal || showBatteryChipVal)
        if (shouldSuppress) {
        ...
        bindHandle = composeView.repeatWhenAttached {
            repeatOnLifecycle(Lifecycle.State.CREATED) {
                combine(
                    ...
                    val onKg = args[0] as Boolean
                    val enabled = args[1] as Boolean
                    val kgEnabled = args[2] as Boolean
                    val mediaEnabled = args[3] as Boolean
                    val lyricsEnabled = args[4] as Boolean
                    val state = args[5] as com.android.systemui.axdynamicbar.ui.AxDynamicBarChipState?
                    val batteryChipMode = args[6] as Int
                    val batteryInfo = args[7] as com.android.systemui.axdynamicbar.ui.KeyguardBatteryInfo
                    val isDozing = args[8] as Boolean

                    val showNormalDynamicBar = enabled && kgEnabled && state != null
                    ...
                    val isBatteryChipEnabled = batteryChipMode > 0
                    val isBatteryChipVisible = isBatteryChipEnabled && (batteryChipMode == 2 || (batteryChipMode == 1 && batteryInfo.isCharging))
                    val showBatteryChip = state == null && isBatteryChipVisible && enabled && kgEnabled && !isDozing

                    onKg && (showNormalDynamicBar || showLockscreenMedia || showLockscreenLyrics || showBatteryChip)
