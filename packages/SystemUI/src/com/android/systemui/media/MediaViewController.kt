...
class MediaViewController @Inject constructor(
...
) : MediaSessionTrackHelper.MediaMetadataListener, ScrimUtils.ScrimEventListener {
    ...
    private fun updateSettings() {
        ...
        if (featureEnabled && !listening) {
            MediaSessionTrackHelper.getInstance(context).addMediaMetadataListener(this)
            ScrimUtils.get().addListener(this)
            listening = true
        } else if (!featureEnabled && listening) {
            MediaSessionTrackHelper.getInstance(context).removeMediaMetadataListener(this)
    ...
    override fun onMediaMetadataChanged() {
        ...
        val bitmap = helper.getMediaBitmap()
        coroutineScope.launch {
            artworkDrawable = bitmap?.let { BitmapDrawable(context.resources, it) }
            onMediaStateChanged()
    ...
    override fun onPlaybackStateChanged() {
        val helper = MediaSessionTrackHelper.getInstance(context)
        isMediaPlaying = helper.isMediaPlaying()
        coroutineScope.launch {
            if (!isMediaPlaying) {
