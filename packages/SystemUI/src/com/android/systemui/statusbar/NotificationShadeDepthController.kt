...
constructor(
    ...
    val updateBlurCallback =
        Choreographer.FrameCallback {
            ...
            val isBlurNegligible = java.lang.Math.abs(blur - lastAppliedBlur) < 2
            val isZoomNegligible = java.lang.Math.abs(zoomOutFromShadeRadius - lastZoomOut) < 0.005f
            val isAtBoundaries = blur <= 1 || blur >= maxBlurRadiusPx - 1 || zoomOutFromShadeRadius <= 0.001f || zoomOutFromShadeRadius >= 0.999f
            if (isBlurNegligible && isZoomNegligible && !isAtBoundaries) {
    ...
    private fun onZoomOutChanged(zoomOutFromShadeRadius: Float) {
        lastZoomOut = zoomOutFromShadeRadius
