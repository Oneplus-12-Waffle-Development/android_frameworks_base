...
private fun VolumeDialogSlider(
    ...
    val isGradientActive = rememberVolumeGradientEnabled()
    val thumbColorOverride: Color? =
        if (!isGradientActive) {
            null
        } else if (com.android.systemui.util.GradientColorUtils.rememberGradientColorMode(isGradientActive) == 1) {
            val (start, _) = com.android.systemui.util.GradientColorUtils.rememberGradientCustomColors(
                isGradientActive,
